...
function initialize() {
  ...
  function describe() {
    ...
    $('care-note').textContent = food ? 'Food is optional. Your cat only needs a bowl while its linked bowl key is visible. Changing pages pauses that need. An empty bowl earns a dramatic face, never illness or death.' : 'Litter care is optional. Your cat only needs cleaning while its linked litter box key is visible. Changing pages pauses that need. A dirty box earns a grumpy face, never illness or death.';
  }
  function render(payload) {
    ...
    $('status-description').textContent = !care.active ? 'Add or select a cat on this page to start this routine.' : kind === 'food' ? care.eating ? care.shared ? 'The cats are eating from one shared bowl.' : 'Your cat is eating. The bowl and cat are sharing this meal.' : care.level <= 0 ? care.shared ? 'The bowl is empty. A refill calls every cat on this page.' : 'The bowl is empty. Refill it and your cat will come for fresh food.' : care.level <= FOOD_LOW_LEVEL ? care.shared ? 'Almost empty. A refill will attract all the cats for a fresh meal.' : 'Almost empty. A refill will attract your cat for a fresh meal.' : 'Ready for your cat’s next little meal.' : readyToClean ? 'Release the key to clean the litter box.' : holding ? 'Keep holding until the bar fills, then release to clean. Releasing early cancels.' : cleaned ? 'The green check confirms the litter box has been cleaned.' : care.using ? 'Occupied. Please respect the pixels.' : care.soil >= LITTER_CAPACITY ? 'Your cat has some opinions about the mess. Hold for 3 seconds, then release to clean.' : care.soil ? 'A little housekeeping would be appreciated.' : 'Clean and ready for a private visit.';
